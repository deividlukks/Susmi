...
import { useCurrentUser } from '@susmi/auth';
import { analyticsService } from '@/services/analytics.service';
import { TasksStatusChart } from '@/components/charts/TasksStatusChart';
import { HabitsCompletionChart } from '@/components/charts/HabitsCompletionChart';
...
export default function DashboardPage() {
  ...
  // Chart data queries
  const { data: tasksStatusData = [] } = useQuery({
    queryKey: ['analytics', 'tasks-status'],
    queryFn: analyticsService.getTasksStatusData,
  });

  const { data: habitsData = [] } = useQuery({
    queryKey: ['analytics', 'habits-performance'],
    queryFn: analyticsService.getHabitsPerformanceData,
  });

  const { data: activityData = [] } = useQuery({
    queryKey: ['analytics', 'activity-timeline'],
    queryFn: () => analyticsService.getActivityTimelineData(7),
  });

  const { data: projectsData = [] } = useQuery({
    queryKey: ['analytics', 'projects-progress'],
    queryFn: analyticsService.getProjectsProgressData,
  });
    ...
    <div>
      ...
      <div className="mt-8 space-y-6">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <TasksStatusChart data={tasksStatusData} />
          <ProjectsProgressChart data={projectsData} />
        </div>

        {/* Habits Chart - Full Width */}
        <div className="w-full">
          <HabitsCompletionChart data={habitsData} />
        </div>
      </div>
    </div>
  );
